        ...
        <form id="examForm" action="{{ route('teacher.exams.update', $exam) }}" method="POST" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 space-y-8 relative">
            ...
            <div>
                <div class="flex items-center justify-between border-b border-gray-100 pb-3 mb-5">
                    ...
                    <span class="text-xs text-gray-500">Mã đề: <strong class="font-mono text-blue-600">{{ $exam->code }}</strong></span>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="category_id" id="exam_category_id" class="w-full rounded-xl border-gray-300 focus:border-blue-500 focus:ring-blue-500 px-4 py-2.5 bg-gray-50 text-sm">
                            ...
                            @foreach($categories as $cat)
                                <option value="{{ $cat->id }}" data-subject="{{ $cat->subject_id }}" {{ (old('category_id', $exam->category_id) == $cat->id) ? 'selected' : '' }}>{{ $cat->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="p-4 bg-blue-50/50 rounded-2xl border border-blue-100/80 md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="datetime-local" id="start_at" name="start_at" value="{{ old('start_at', $exam->start_at ? $exam->start_at->format('Y-m-d\TH:i') : '') }}" class="w-full rounded-xl border-gray-300 focus:border-blue-500 focus:ring-blue-500 px-4 py-2.5 bg-white text-sm">
                            <p class="text-[11px] text-gray-500 mt-1">Thời điểm sinh viên có thể vào thi</p>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="datetime-local" id="end_at" name="end_at" value="{{ old('end_at', $exam->end_at ? $exam->end_at->format('Y-m-d\TH:i') : '') }}" class="w-full rounded-xl border-gray-300 focus:border-blue-500 focus:ring-blue-500 px-4 py-2.5 bg-white text-sm">
                    ...
                    <div>
                        ...
                        <input type="number" id="duration_minutes" name="duration_minutes" value="{{ old('duration_minutes', $exam->duration_minutes) }}" min="1" required class="w-full rounded-xl border-gray-300 focus:border-blue-500 focus:ring-blue-500 px-4 py-2.5 bg-gray-50 text-sm font-semibold text-gray-900">
                    ...
                    <div>
                        ...
                        <div class="space-y-2">
                            <input type="number" id="max_attempts_input" name="max_attempts" value="{{ old('max_attempts', $exam->max_attempts) }}" min="1" {{ $exam->isUnlimitedAttempts() ? 'disabled' : '' }} class="w-full rounded-xl border-gray-300 focus:border-blue-500 focus:ring-blue-500 px-4 py-2.5 {{ $exam->isUnlimitedAttempts() ? 'bg-gray-100 text-gray-400' : 'bg-gray-50' }} text-sm">
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" id="unlimited_attempts_cb" name="unlimited_attempts" value="1" {{ old('unlimited_attempts', $exam->isUnlimitedAttempts()) ? 'checked' : '' }} onchange="toggleUnlimitedAttempts(this.checked)" class="text-blue-600 focus:ring-blue-500 rounded text-sm w-4 h-4">
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-gray-800 mb-2">Trạng thái <span class="text-red-500">*</span></label>
                        <select name="status" class="w-full rounded-xl border-gray-300 focus:border-blue-500 focus:ring-blue-500 px-4 py-2.5 bg-gray-50 text-sm font-semibold">
                            <option value="published" {{ old('status', $exam->status) === 'published' ? 'selected' : '' }}>Mở (Published)</option>
                            <option value="closed" {{ old('status', $exam->status) === 'closed' ? 'selected' : '' }}>Đóng (Closed)</option>
                        </select>
                    </div>
                    ...
                    <div class="md:col-span-2 bg-gradient-to-br from-slate-50 to-indigo-50/40 rounded-2xl p-5 border border-indigo-100/80 space-y-4">
                        <div class="flex items-center justify-between border-b border-indigo-100/60 pb-3 flex-wrap gap-2">
                            ...
                            <label class="flex items-center gap-2 cursor-pointer bg-white px-3 py-1.5 rounded-xl border border-indigo-200 shadow-sm">
                                <input type="checkbox" id="enable_anti_cheat" name="enable_anti_cheat" value="1" {{ old('enable_anti_cheat', $exam->enable_anti_cheat ?? true) ? 'checked' : '' }} onchange="toggleAllAntiCheat(this.checked)" class="text-indigo-600 focus:ring-indigo-500 rounded text-sm w-4 h-4">
                                <span class="text-xs font-bold text-indigo-900">Bật Giám sát Chống Gian lận</span>
                            </label>
                        </div>

                        <div id="antiCheatSubOptions" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5 pt-1 {{ old('enable_anti_cheat', $exam->enable_anti_cheat ?? true) ? '' : 'opacity-40 pointer-events-none' }}">
                            <label class="flex items-start p-3 bg-white rounded-xl border border-gray-200 hover:border-indigo-300 transition-colors cursor-pointer gap-2.5">
                                <input type="checkbox" name="require_fullscreen" value="1" {{ old('require_fullscreen', $exam->require_fullscreen ?? true) ? 'checked' : '' }} class="anti-cheat-sub mt-0.5 text-indigo-600 focus:ring-indigo-500 rounded text-sm w-4 h-4">
                                <div>
                            ...
                            <label class="flex items-start p-3 bg-white rounded-xl border border-gray-200 hover:border-indigo-300 transition-colors cursor-pointer gap-2.5">
                                <input type="checkbox" name="prevent_tab_switch" value="1" {{ old('prevent_tab_switch', $exam->prevent_tab_switch ?? true) ? 'checked' : '' }} class="anti-cheat-sub mt-0.5 text-indigo-600 focus:ring-indigo-500 rounded text-sm w-4 h-4">
                                <div>
                            ...
                            <label class="flex items-start p-3 bg-white rounded-xl border border-gray-200 hover:border-indigo-300 transition-colors cursor-pointer gap-2.5">
                                <input type="checkbox" name="prevent_copy_paste" value="1" {{ old('prevent_copy_paste', $exam->prevent_copy_paste ?? true) ? 'checked' : '' }} class="anti-cheat-sub mt-0.5 text-indigo-600 focus:ring-indigo-500 rounded text-sm w-4 h-4">
                                <div>
                            ...
                            <label class="flex items-start p-3 bg-white rounded-xl border border-gray-200 hover:border-indigo-300 transition-colors cursor-pointer gap-2.5">
                                <input type="checkbox" name="prevent_right_click" value="1" {{ old('prevent_right_click', $exam->prevent_right_click ?? true) ? 'checked' : '' }} class="anti-cheat-sub mt-0.5 text-indigo-600 focus:ring-indigo-500 rounded text-sm w-4 h-4">
                                <div>
                            ...
                            <label class="flex items-start p-3 bg-white rounded-xl border border-gray-200 hover:border-indigo-300 transition-colors cursor-pointer gap-2.5">
                                <input type="checkbox" name="prevent_screen_capture" value="1" {{ old('prevent_screen_capture', $exam->prevent_screen_capture ?? true) ? 'checked' : '' }} class="anti-cheat-sub mt-0.5 text-indigo-600 focus:ring-indigo-500 rounded text-sm w-4 h-4">
                                <div>
                            ...
                            <label class="flex items-start p-3 bg-white rounded-xl border border-indigo-200 hover:border-indigo-400 transition-colors cursor-pointer gap-2.5">
                                <input type="checkbox" name="require_face_verification" value="1" {{ old('require_face_verification', $exam->require_face_verification ?? true) ? 'checked' : '' }} class="anti-cheat-sub mt-0.5 text-indigo-600 focus:ring-indigo-500 rounded text-sm w-4 h-4">
                                <div>
                            ...
                            <div id="proctor_camera_container" class="sm:col-span-2 lg:col-span-3 grid grid-cols-1 {{ old('enable_proctor_camera', $exam->enable_proctor_camera ?? true) ? 'lg:grid-cols-2' : '' }} gap-3.5 items-stretch">
                                ...
                                <label id="proctor_cam_card" class="flex items-start p-3.5 bg-white rounded-xl border border-indigo-200 hover:border-indigo-400 transition-colors cursor-pointer gap-2.5 h-full shadow-2xs">
                                    <input type="checkbox" name="enable_proctor_camera" id="toggle_proctor_camera" value="1" {{ old('enable_proctor_camera', $exam->enable_proctor_camera ?? true) ? 'checked' : '' }} onchange="toggleProctorCameraSetting(this.checked)" class="anti-cheat-sub mt-0.5 text-indigo-600 focus:ring-indigo-500 rounded text-sm w-4 h-4 shrink-0">
                                    <div>
                                        <span class="block text-xs font-bold text-indigo-900 flex items-center gap-1.5">
                                ...
                                <div id="proctor_interval_box" class="p-3.5 bg-gradient-to-r from-indigo-50/90 to-blue-50/90 rounded-xl border border-indigo-200 shadow-xs transition-all h-full flex flex-col justify-between" style="{{ old('enable_proctor_camera', $exam->enable_proctor_camera ?? true) ? '' : 'display:none;' }}">
            ...
            <div>
                <div class="flex items-center justify-between border-b border-gray-100 pb-3 mb-5">
                    <div>
                        ...
                        <p class="text-xs text-gray-500">Chỉnh sửa danh sách câu hỏi trong đề thi</p>
                    </div>
                ...
                <div id="selectedQuestionsContainer" class="hidden">
                    ...
                    <div class="flex justify-between items-center text-sm font-medium text-gray-700 bg-gray-50 p-4 rounded-xl border border-gray-100">
                        <span class="flex items-center gap-1.5">
                            ...
                            Tổng số câu: <span id="totalQuestionsCount" class="font-bold text-blue-700">0</span>
                        </span>
                        <span class="flex items-center gap-1.5">
                ...
                <div class="bg-gray-50 border border-dashed border-gray-300 rounded-xl p-8 text-center" id="emptyQuestions">
                    ...
                    <p class="mt-1 text-sm text-gray-500">Hãy bắt đầu bằng việc chọn từ ngân hàng đề hoặc tạo câu hỏi mới.</p>
                </div>
            </div>
    ...
    <script>
        ...
        let selectedQuestions = [
            @foreach($exam->questions as $q)
            {
                ...
                category_name: {!! json_encode($q->category ? $q->category->name : null) !!}
            },
            @endforeach
        ];
        ...
        function renderBankGroupedQuestions() {
            ...
            Object.values(groups).forEach(group => {
                ...
                table.innerHTML = `<tbody class="divide-y divide-gray-100 bg-white"></tbody>`;
