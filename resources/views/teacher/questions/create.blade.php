...
@section('content')
    <div class="max-w-4xl mx-auto">
        <form action="{{ route('teacher.questions.store') }}" method="POST" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 space-y-6">
            @csrf
                ...
                <div>
                    ...
                    <select name="category_id" id="category_id" class="w-full rounded-xl border-gray-300 focus:border-blue-500 focus:ring-blue-500 px-4 py-2.5 bg-gray-50 text-sm">
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" data-subject="{{ $category->subject_id }}" {{ old('category_id') == $category->id ? 'selected' : '' }}>
                                {{ $category->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div>
                ...
                <textarea name="content" rows="4" required class="w-full rounded-xl border-gray-300 focus:border-blue-500 focus:ring-blue-500 px-4 py-3 bg-gray-50 text-sm" placeholder="Nhập câu hỏi tại đây...">{{ old('content') }}</textarea>
            </div>
            ...
            <div>
                ...
                <div class="flex items-center gap-6">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="difficulty" value="easy" class="text-emerald-600 focus:ring-emerald-500 w-4 h-4" {{ old('difficulty', 'medium') == 'easy' ? 'checked' : '' }}>
                        <span class="text-sm font-medium text-gray-700">Dễ</span>
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="difficulty" value="medium" class="text-amber-600 focus:ring-amber-500 w-4 h-4" {{ old('difficulty', 'medium') == 'medium' ? 'checked' : '' }}>
                        <span class="text-sm font-medium text-gray-700">Trung bình</span>
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="difficulty" value="hard" class="text-rose-600 focus:ring-rose-500 w-4 h-4" {{ old('difficulty', 'medium') == 'hard' ? 'checked' : '' }}>
            ...
            <div id="multipleChoiceSection" class="p-6 bg-gray-50 rounded-xl border border-gray-200">
                ...
                <div id="answersList" class="space-y-4">
                    @for($i = 0; $i < 4; $i++)
                        <div class="flex items-start gap-4 answer-row">
                            <div class="pt-3">
                                <input type="radio" name="correct_answer" value="{{ $i }}" class="w-5 h-5 text-blue-600 focus:ring-blue-500 cursor-pointer" {{ old('correct_answer', 0) == $i ? 'checked' : '' }}>
                            </div>
            ...
            <div>
                ...
                <textarea name="explanation" rows="2" class="w-full rounded-xl border-gray-300 focus:border-blue-500 focus:ring-blue-500 px-4 py-3 bg-gray-50 text-sm" placeholder="Giải thích đáp án cho học viên...">{{ old('explanation') }}</textarea>
            </div>
