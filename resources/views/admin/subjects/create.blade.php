...
@section('content')
    <div class="max-w-2xl mx-auto">
        <form action="{{ route('admin.subjects.store') }}" method="POST" class="bg-white rounded-2xl shadow-[0_2px_10px_-3px_rgba(6,81,237,0.1)] border border-gray-100 p-8 space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="code" value="{{ old('code') }}" required class="w-full rounded-xl border-slate-300 focus:border-indigo-500 focus:ring-indigo-500 px-4 py-2.5 bg-slate-50 text-sm uppercase" placeholder="VD: CS101">
                </div>
                
                <!-- Name -->
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">Tên môn học <span class="text-rose-500">*</span></label>
                    <input type="text" name="name" value="{{ old('name') }}" required class="w-full rounded-xl border-slate-300 focus:border-indigo-500 focus:ring-indigo-500 px-4 py-2.5 bg-slate-50 text-sm" placeholder="VD: Lập trình C++">
                </div>

                <!-- Credits -->
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">Số tín chỉ (TC) <span class="text-rose-500">*</span></label>
                    <input type="number" name="credits" min="1" max="15" value="{{ old('credits', 3) }}" required class="w-full rounded-xl border-slate-300 focus:border-indigo-500 focus:ring-indigo-500 px-4 py-2.5 bg-slate-50 text-sm" placeholder="3">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="description" rows="3" class="w-full rounded-xl border-slate-300 focus:border-indigo-500 focus:ring-indigo-500 px-4 py-3 bg-slate-50 text-sm" placeholder="Giới thiệu về môn học...">{{ old('description') }}</textarea>
            </div>
                            ...
                            <div class="space-y-1">
                                @foreach($teachers as $teacher)
                                    <label class="teacher-item flex items-center gap-3 p-2 hover:bg-slate-100 rounded-lg cursor-pointer" data-search="{{ strtolower($teacher->name . ' ' . $teacher->email . ' ' . $department) }}">
                                        <input type="checkbox" name="teachers[]" value="{{ $teacher->id }}" class="w-4 h-4 text-indigo-600 rounded border-slate-300 focus:ring-indigo-500 cursor-pointer" {{ (is_array(old('teachers')) && in_array($teacher->id, old('teachers'))) ? 'checked' : '' }}>
                                        <div>
            ...
            <div class="flex items-center gap-3 pt-2">
                <input type="checkbox" name="status" id="status" value="1" class="w-5 h-5 text-indigo-600 rounded border-slate-300 focus:ring-indigo-500 cursor-pointer" {{ old('status', true) ? 'checked' : '' }}>
