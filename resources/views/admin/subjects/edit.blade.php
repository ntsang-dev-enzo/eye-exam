...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <h1 class="text-xl font-bold text-slate-900 tracking-tight">Cập nhật Môn Học: {{ $subject->name }}</h1>
        </div>
    ...
    <form action="{{ route('admin.subjects.update', $subject) }}" method="POST" class="bg-white rounded-lg border border-slate-200 p-6 space-y-5">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
            ...
            <div>
                ...
                <input type="text" name="code" value="{{ old('code', $subject->code) }}" required 
                    class="w-full h-10 px-3 bg-white border border-slate-200 rounded-md text-sm font-medium text-slate-900 uppercase focus:outline-none focus:border-blue-600 focus:ring-1 focus:ring-blue-600 transition-colors">
            </div>
            ...
            <div>
                ...
                <input type="number" name="credits" min="1" max="15" value="{{ old('credits', $subject->credits ?? 3) }}" required 
                    class="w-full h-10 px-3 bg-white border border-slate-200 rounded-md text-sm font-medium text-slate-900 focus:outline-none focus:border-blue-600 focus:ring-1 focus:ring-blue-600 transition-colors">
            </div>
        </div>
        ...
        <div>
            ...
            <textarea name="description" rows="3" 
                class="w-full p-3 bg-white border border-slate-200 rounded-md text-sm font-medium text-slate-900 placeholder-slate-400 focus:outline-none focus:border-blue-600 focus:ring-1 focus:ring-blue-600 transition-colors">{{ old('description', $subject->description) }}</textarea>
        </div>
        ...
        <div class="space-y-2">
            ...
            <p class="text-[11px] text-slate-500">* Các giảng viên được tick chọn sẽ có quyền quản lý ngân hàng câu hỏi và tạo đề thi cho môn học này.</p>
        </div>
