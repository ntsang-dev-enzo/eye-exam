...
<div class="max-w-2xl mx-auto">
    <div class="flex items-center gap-2 mb-6">
        ...
        <span class="text-sm font-medium text-slate-900">Sửa đổi</span>
    </div>

    <form action="{{ route('admin.classes.update', $class) }}" method="POST" class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        @csrf
        @method('PUT')
        
        <div class="px-8 py-6 border-b border-slate-200 bg-slate-50/50">
            <h3 class="text-lg font-bold text-slate-800">Cập nhật Lớp học: {{ $class->code }}</h3>
        </div>
        ...
        <div class="p-8 space-y-6">
            <div class="grid grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="code" value="{{ old('code', $class->code) }}" required class="w-full px-4 py-2.5 rounded-xl border border-slate-300 focus:ring-indigo-500 focus:border-indigo-500 shadow-sm text-sm uppercase">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $class->name) }}" required class="w-full px-4 py-2.5 rounded-xl border border-slate-300 focus:ring-indigo-500 focus:border-indigo-500 shadow-sm text-sm">
            ...
            <div>
                ...
                <textarea name="description" rows="3" class="w-full px-4 py-2.5 rounded-xl border border-slate-300 focus:ring-indigo-500 focus:border-indigo-500 shadow-sm text-sm">{{ old('description', $class->description) }}</textarea>
            </div>
        </div>
        ...
        <div class="px-8 py-5 border-t border-slate-200 bg-slate-50 flex items-center justify-end gap-3">
            ...
            <button type="submit" class="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold rounded-xl shadow-sm transition-colors">Cập nhật</button>
        </div>
    </form>
</div>
@endsection
