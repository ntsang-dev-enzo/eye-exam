...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <h1 class="text-2xl font-black text-slate-900 tracking-tight">Thêm Khóa Học Mới</h1>
        </div>
    ...
    <form method="POST" action="{{ route('admin.khoa-hoc.store') }}" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="description" name="description" rows="3" placeholder="Nhập mô tả ngắn gọn về khóa học..." 
                        class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium text-slate-800 placeholder-slate-400 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-all">{{ old('description') }}</textarea>
                </div>
                ...
                <div class="md:col-span-2">
                    <label class="flex items-center gap-3 cursor-pointer select-none">
                        ...
                        <div>
                            <span class="block text-sm font-bold text-slate-800">Kích hoạt khóa học</span>
                            <span class="block text-xs text-slate-400">Khóa học sau khi tạo sẽ ở trạng thái đang hoạt động</span>
                        </div>
                    </label>
                </div>
            </div>
        ...
        <!-- Subject Picker Component (CRITICAL) -->
        <div class="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-sm space-y-4"
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-100 pb-4 gap-2">
                <div>
                    ...
                    <p class="text-xs text-slate-500 font-medium">Chọn các môn học thuộc chương trình của khóa học này</p>
                </div>
