...
| **Xác thực Face ID** | Quét tự động trước giờ thi (Zero-Click) | JavaScript Canvas Loop, AJAX API | Cosine Similarity (Ngưỡng an toàn $\ge 70\%$) | Thí sinh căn khuôn mặt vào vòng tròn trung tâm, hệ thống tự động phát hiện, chụp và so khớp với vector gốc. Trùng khớp sẽ tự động chuyển vào bài thi ngay lập tức mà không cần bấm nút. Tự động quét lại sau 1.2s nếu thất bại. |
...
| **Chụp Ảnh Định Kỳ** | Snapshot ngẫu nhiên trong bài thi | WebRTC MediaStream + Canvas | Random Interval Algorithm: $90s - 120s$ | Tự động chụp ngầm định kỳ ngẫu nhiên từ 1 phút 30 giây đến 2 phút/lần. Widget camera thu nhỏ (PIP) cố định góc dưới bên trái màn hình. |
...
| **Bảo Mật & Riêng Tư**| Bảo mật Avatar & Thông tin cá nhân sinh viên | Role-Based Access Control + Attribute Hiding | `role:student` middleware + `$hidden` biometric vectors | Avatar khuôn mặt của sinh viên được bảo vệ tuyệt đối (sinh viên khác không thể xem trộm), ẩn vĩnh viễn vector sinh trắc học và đường dẫn nội bộ khỏi JSON, che chắn danh sách lớp chỉ hiển thị họ tên cơ bản. |
